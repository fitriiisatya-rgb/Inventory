...
final class ImportOpeningStockService
{
    ...
    // PHASE V2.6A: the .xlsx parsing itself now lives in XlsxReaderService
    // (extracted verbatim, byte-for-byte identical behavior) so
    // ImportMasterItemService/ImportSimpleMasterService can reuse the exact
    // same proven parser instead of only this importer supporting .xlsx.
    private static function readXlsx(string $path): array
    {
        return XlsxReaderService::read($path);
    }
}
